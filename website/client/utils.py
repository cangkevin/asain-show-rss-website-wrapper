...
def extract_paginations(entries):
    ...
    pagination_links = [
        entry for entry in entries if re.match(pagination_pattern, entry["title"])
    ]
    entries[:] = [
        entry for entry in entries if not re.match(pagination_pattern, entry["title"])
    ]
...
def extract_id(entry):
    return os.path.normpath(urlsplit(entry.links[0].href).path).split(os.sep).pop()
...
def extract_episodes(data):
    return [{"title": entry.title, "id": extract_id(entry)} for entry in data.entries]


def extract_sources(data):
    sources = [
        {"title": entry.title, "url": entry.links[0].href} for entry in data.entries
    ]

    return sorted(
        list(unique_everseen(sources, key=lambda e: "{url}".format(**e))),
