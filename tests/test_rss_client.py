...
def test_get_movies(mocked_response):
    with open(Path(MOVIES_RESP_FILE), 'rb') as resp:
        mocked_response.add(
            responses.GET, client.utils.build_movies_uri('hk-movies', '1'),
            body=resp, status=200)
        rss_resp = client.get_movies('hk-movies', '1')
...
def test_get_shows(mocked_response):
    with open(Path(SHOWS_RESP_FILE), 'rb') as resp:
        mocked_response.add(
            responses.GET, client.utils.build_shows_uri('hk-drama', '1'),
            body=resp, status=200)
...
def test_get_episodes(mocked_response):
    with open(Path(EPISODES_RESP_FILE), 'rb') as resp:
        mocked_response.add(
            responses.GET, client.utils.build_episodes_uri('12345', '1'),
            body=resp, status=200)
        rss_resp = client.get_episodes('12345', '1')
...
def test_get_sources(mocked_response):
    with open(Path(SOURCES_RESP_FILE), 'rb') as resp:
        mocked_response.add(
            responses.GET, client.utils.build_sources_uri('99999'),
            body=resp, status=200)
        rss_resp = client.get_sources('99999')
...
def test_get_movies_timed_out(mocked_response):
    ...
    with pytest.raises(client.exceptions.ClientTimeoutError):
        client.get_movies('hk-movies', '1')
...
def test_get_sources_timed_out(mocked_response):
    ...
    with pytest.raises(client.exceptions.ClientTimeoutError):
        client.get_sources('99999')


def test_get_invalid_movie(mocked_response):
    with open(Path(EMPTY_RESP_FILE), 'rb') as resp:
        mocked_response.add(
            responses.GET, client.utils.build_movies_uri('invalid-movie', '1'),
            body=resp, status=200
        )

        with pytest.raises(client.exceptions.InvalidResourceError):
            client.get_movies('invalid-movie', '1')


def test_get_invalid_show(mocked_response):
    with open(Path(EMPTY_RESP_FILE), 'rb') as resp:
        mocked_response.add(
            responses.GET, client.utils.build_shows_uri('invalid-show', '1'),
            body=resp, status=200
        )

        with pytest.raises(client.exceptions.InvalidResourceError):
            client.get_shows('invalid-show', '1')


def test_get_invalid_episode(mocked_response):
    with open(Path(EMPTY_RESP_FILE), 'rb') as resp:
        mocked_response.add(
            responses.GET, client.utils.build_episodes_uri('invalid-ep', '1'),
            body=resp, status=200
        )

        with pytest.raises(client.exceptions.InvalidResourceError):
            client.get_episodes('invalid-ep', '1')


def test_get_invalid_sources(mocked_response):
    with open(Path(EMPTY_RESP_FILE), 'rb') as resp:
        mocked_response.add(
            responses.GET, client.utils.build_sources_uri('invalid-source'),
            body=resp, status=200
        )

        with pytest.raises(client.exceptions.InvalidResourceError):
            client.get_sources('invalid-source')
