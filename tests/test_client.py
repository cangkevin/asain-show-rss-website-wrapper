...
def test_get_movies(mocked_response):
    with open(Path(MOVIES_RESP_FILE), "rb") as resp:
        mocked_response.add(
            responses.GET,
            client.utils.build_movies_uri("hk-movies", "1"),
            body=resp,
            status=200,
        )
...
def test_get_shows(mocked_response):
    with open(Path(SHOWS_RESP_FILE), "rb") as resp:
        mocked_response.add(
            responses.GET,
            client.utils.build_shows_uri("hk-drama", "1"),
            body=resp,
            status=200,
        )
...
def test_get_episodes(mocked_response):
    with open(Path(EPISODES_RESP_FILE), "rb") as resp:
        mocked_response.add(
            responses.GET,
            client.utils.build_episodes_uri("12345", "1"),
            body=resp,
            status=200,
        )
...
def test_get_sources(mocked_response):
    with open(Path(SOURCES_RESP_FILE), "rb") as resp:
        mocked_response.add(
            responses.GET,
            client.utils.build_sources_uri("99999"),
            body=resp,
            status=200,
        )
...
def test_get_invalid_movie(mocked_response):
    with open(Path(EMPTY_RESP_FILE), "rb") as resp:
        mocked_response.add(
            responses.GET,
            client.utils.build_movies_uri("invalid-movie", "1"),
            body=resp,
            status=200,
        )
...
def test_get_invalid_show(mocked_response):
    with open(Path(EMPTY_RESP_FILE), "rb") as resp:
        mocked_response.add(
            responses.GET,
            client.utils.build_shows_uri("invalid-show", "1"),
            body=resp,
            status=200,
        )
...
def test_get_invalid_episode(mocked_response):
    with open(Path(EMPTY_RESP_FILE), "rb") as resp:
        mocked_response.add(
            responses.GET,
            client.utils.build_episodes_uri("invalid-ep", "1"),
            body=resp,
            status=200,
        )
...
def test_get_invalid_sources(mocked_response):
    with open(Path(EMPTY_RESP_FILE), "rb") as resp:
        mocked_response.add(
            responses.GET,
            client.utils.build_sources_uri("invalid-source"),
            body=resp,
            status=200,
        )
